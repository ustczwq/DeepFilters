...
class FilterVisualizer(object):
    ...
    def hookLayer(self, depth):
        def hookFunc(module, gradIn, gradOut):
            self.features = gradOut
        self.model[depth].register_forward_hook(hookFunc)

    def visualize(self, depth, channel, lr=0.1, epochs=20):
        self.hookLayer(depth)
        ...
        for i in range(self.upSteps):
            ...
            for ep in range(epochs):
                ...
                self.model(ten)
                loss = -torch.mean(self.features[0, channel])
    ...
    def imgSave(self, depth, channel):
        folder = './outputs/' + str(depth) + '/'
        if not os.path.exists(folder):
            os.makedirs(folder)

        img = self.img
        plt.imsave(folder + str(depth) + '_' + str(channel) + '.jpg', img)
...
for c in range(10):
    FV.visualize(depth, channel + c, lr=0.05, epochs=40)
